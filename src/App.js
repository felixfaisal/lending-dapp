...
function App() {
  ...
  const [walletFGSBalance, setWalletFGSBalance] = useState("");
  ...
  const checkTokenBalances = async () => {
    try {
      ...
      const fgsBalance = await stableTokenContract.balanceOf(
        signer.getAddress()
      );
      if (!fgsBalance) {
        addTokenFunction();
      } else {
        setWalletFGSBalance(fgsBalance / 10 ** 18);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  useEffect(() => {
    checkTokenBalances();
  }, [walletEthBalance, walletFGSBalance]);

  async function addTokenFunction() {
    try {
      const tokenAddress = "0x56b7e59990480a176174B882DA839bFaA14C4ac4";
      const tokenSymbol = "FGS";
      ...
      <section className="w-full flex flex-col justify-start items-center  mt-4 my-4">
        <form className="p-5 sm:w-96 w-full flex flex-col justify-start items-center blue-glassmorphism">
          ...
          <p className="text-white font-bold mt-10 mb-3">
            FGS Balance: {walletFGSBalance.toString()}
          </p>
